...
| `concatItems(arr)` | `array (with string/object/QubeItem)` | This concats new `items` as array. Use this method to add one or more new items to the existing items or use this to refill an empty inventory of items. The parameter `arr` defines an array with strings, custom objects or objects with the object structure of QubeItem. |
